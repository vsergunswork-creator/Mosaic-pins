...
// GET  /api/stripe-webhook?ship_check=1&secret=XXX  -> cron: send "shipped" emails from Airtable (no automations)

// Listens: checkout.session.completed
// 1) UPSERT order in Airtable Orders by "Stripe Session ID"
// 2) Decrement stock in Airtable Products (idempotent via KV)
// 3) ✅ Cron ship_check: find orders with Tracking Number and send email once, then tick "Shipped Email Sent"
...
export async function onRequestPost(ctx) {
  ...
  try {
    ...
    // Products table
    const PRODUCTS_TABLE = String(env.AIRTABLE_TABLE_NAME || "Products");
    // Orders table
    ...
    if (!eventId) return json({ received: true, note: "Missing event.id" }, 200);
    ...
    if (paymentStatus !== "paid") {
      await env.STRIPE_EVENTS_KV.put(EVT_KEY, "stock_done", { expirationTtl: 30 * 24 * 60 * 60 });
      return json({ received: true, ignored: true, reason: "payment_status_not_paid" }, 200);
    }
    ...
    if (!itemsJson) {
      await env.STRIPE_EVENTS_KV.put(EVT_KEY, "stock_done", { expirationTtl: 30 * 24 * 60 * 60 });
      return json({ received: true, note: "No metadata.items" }, 200);
    }
    ...
    // ---------- normalize items ----------
    // expected item format: { recordId, pin, qty }
    ...
    if (!normalized.length) {
      await env.STRIPE_EVENTS_KV.put(EVT_KEY, "stock_done", { expirationTtl: 30 * 24 * 60 * 60 });
      return json({ received: true, note: "No valid recordId/qty in items" }, 200);
    }
    ...
    // IMPORTANT: field names must match your Airtable exactly
    const orderFields = {
    ...
    if (alreadyStockDone) {
      await env.STRIPE_EVENTS_KV.put(EVT_KEY, "stock_done", { expirationTtl: 30 * 24 * 60 * 60 });
      return json({ received: true, upserted: true, stock: "skipped_already_done" }, 200);
    }
    ...
    await env.STRIPE_EVENTS_KV.put(EVT_KEY, "stock_done", { expirationTtl: 30 * 24 * 60 * 60 });
    return json({ received: true, upserted: true, stock: "decremented" }, 200);
  } catch (e) {
...
export async function onRequestGet(ctx) {
  ...
  try {
    ...
    // Airtable fields (from your settings screenshot)
    const TRACKING_FIELD = String(env.AIRTABLE_TRACKING_FIELD || "Tracking Number").trim();
    const SHIPPED_FIELD = String(env.AIRTABLE_SHIPPED_FIELD || "Shipped Email Sent").trim();

    // Mail settings
    const STORE_NAME = String(env.STORE_NAME || "Mosaic Pins").trim();
    const STORE_URL = String(env.STORE_URL || "https://mosaicpins.space").trim();
    ...
    for (const rec of records) {
      ...
      // Extra idempotency via KV (for retries / double crons)
