...
export async function onRequestGet({ env, request }) {
  try {
    ...
    const items = products.filter(p=>p.active).map(p=>{
      ...
      return {id:p.pin,title:p.title||p.pin,description:[cleanText(p.description),extra.join(" • ")].filter(Boolean).join("\n\n").slice(0,5000),link:`${baseUrl}/p/${encodeURIComponent(p.pin).replace(/%2C/gi, ",")}`,image_link:p.images[0],availability:Number(p.stock||0)>0?'in stock':'out of stock',price:`${price.toFixed(2)} USD`,brand:'Mosaic Pins Space',condition:'new',gender:'unisex',age_group:'adult',color:p.color||'Multicolor'};
    }).filter(Boolean);
