...
export async function onRequestPost({ request, env }) {
  try {
    ...
    const html =
      ...
      `<div style="color:#a8b3c7;font-size:13px;line-height:1.5;">` +
    ...
    try {
      ...
      await env.DB.prepare(
        "DELETE FROM account_login_codes WHERE id = ?1"
      ).bind(codeId).run().catch(() => {});
    ...
    return json({
      ok: false,
      error: "Unable to send sign-in code right now."
    }, 500);
  }
}
