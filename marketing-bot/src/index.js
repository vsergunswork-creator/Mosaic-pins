...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/library/ai-album" && request.method === "GET") {
      if (!(await isLibraryAuthorized(request, env))) return json({ ok: false, error: "Unauthorized" }, 401);
      return libraryAiAlbum(env);
    }
...
function libraryAiHistoryButtonHtml(row, bucket) {
  if (bucket !== "ready") return "";
  const id = Number(row.id);
  return `<button class="action ghost" data-action="ai_history" data-id="${id}">🖼 История AI</button><button class="action ghost" data-action="ai_album" data-id="${id}">🗂 Альбом AI</button>`;
}
...
@media(max-width:1050px){
  :root{--dashboard-width:100%}
  .wrap{padding:22px}
  .tabs{grid-template-columns:repeat(3,minmax(0,1fr))}
  .card{grid-template-columns:220px minmax(0,1fr)}
}
@media(max-width:700px){
  .wrap{padding:14px}
  .header{margin-bottom:12px}
  .rotation{align-items:flex-start;min-height:0;padding:11px 12px}
  .dashboard-widgets{grid-template-columns:1fr;gap:9px}
  .widget{min-height:0;padding:12px 13px}
  .editorial{padding:11px 12px}
  .toolbar{grid-template-columns:1fr;grid-template-areas:"tabs" "search" "create" "bulk";gap:9px;padding-top:8px}
  .tabs{display:flex;gap:7px;overflow:auto;padding-bottom:2px}
  .tab{width:auto;min-width:max-content;min-height:38px;border-radius:10px}
  .new-topic-row .create{height:44px}
  .card{display:block;border-radius:14px}
  .body{padding:14px}
  .media{padding:12px}
}
</style></head>
...
<div class="modal" id="aiModal"><div class="modal-box small"><div class="modal-head"><h3 id="aiModalTitle">🖼 История AI</h3><button class="close" id="closeAiModal">Закрыть</button></div><div class="ai-history-grid" id="aiHistory"><div class="loader">Загрузка…</div></div></div></div><div id="toast" class="toast"></div>
<script>(()=>{
const tabs=[...document.querySelectorAll('.tab')],cards=[...document.querySelectorAll('.card')],search=document.getElementById('search'),empty=document.getElementById('empty'),toast=document.getElementById('toast'),modal=document.getElementById('productModal'),products=document.getElementById('products'),aiModal=document.getElementById('aiModal'),aiModalTitle=document.getElementById('aiModalTitle'),aiHistory=document.getElementById('aiHistory'),bulkbar=document.getElementById('bulkbar'),bulkCount=document.getElementById('bulkCount');let bucket='ready',modalPostId=0;
...
async function deleteAiHistoryItem(btn,targetPostId,historyId,item){if(!confirm('Удалить это AI-фото из альбома и D1? Действие нельзя отменить.'))return;const old=btn.textContent;btn.classList.add('busy');btn.textContent='Удаляю…';try{const data=await action('ai_delete_history',targetPostId,{historyId});say(data.message||'AI фото удалено');item.remove();if(!aiHistory.querySelector('.ai-history-item'))aiHistory.innerHTML='<div class="loader">Сохранённых AI вариантов пока нет</div>'}catch(e){say(e.message||'Ошибка удаления',true);btn.classList.remove('busy');btn.textContent=old}}
function renderAiItems(items,targetPostId,mode){aiHistory.innerHTML='';for(const h of (items||[])){const item=document.createElement('div');item.className='ai-history-item';const img=document.createElement('img');img.src='/library/history-media?id='+encodeURIComponent(h.id);img.loading='lazy';img.alt=h.title||'AI image';const small=document.createElement('small');small.textContent=mode==='album'?'Пост #'+Number(h.postId||0)+' · '+(h.createdAt||''):(h.createdAt||'');item.append(img,small);if(mode==='album'&&h.topic){const source=document.createElement('div');source.className='ai-album-source';source.textContent=h.topic;item.append(source)}const check=document.createElement('div');check.className='ai-history-check '+(h.checkStatus||'unknown');const checkTitle=document.createElement('b');const icon=h.checkStatus==='ok'?'✅':h.checkStatus==='warning'?'⚠️':h.checkStatus==='reject'?'⛔':'❔';checkTitle.textContent=icon+' AI image check: '+String(h.checkStatus||'unknown').toUpperCase()+(h.checkScore==null?'':' · '+h.checkScore+'/100');check.append(checkTitle);if(h.checkSummary){const sum=document.createElement('span');sum.textContent=h.checkSummary;check.append(sum)}if(Array.isArray(h.checkIssues)&&h.checkIssues.length){const ul=document.createElement('ul');for(const issue of h.checkIssues.slice(0,4)){const li=document.createElement('li');li.textContent=issue;ul.append(li)}check.append(ul)}const buttons=document.createElement('div');buttons.className='ai-history-buttons';const pick=document.createElement('button');pick.className='action primary';pick.textContent=h.checkStatus==='reject'?'⛔ Reject, выбор заблокирован':h.checkStatus==='warning'?'⚠️ Выбрать всё равно':'✅ Выбрать для этого поста';if(h.checkStatus==='reject'){pick.disabled=true;pick.classList.add('disabled')}else pick.addEventListener('click',()=>runButton(pick,mode==='album'?'ai_select_album':'ai_select_history',targetPostId,{historyId:h.id}));const del=document.createElement('button');del.className='action delete delete-ai';del.textContent='🗑 Удалить';del.addEventListener('click',()=>deleteAiHistoryItem(del,targetPostId,h.id,item));buttons.append(pick,del);item.append(check,buttons);aiHistory.append(item)}if(!(items||[]).length)aiHistory.innerHTML='<div class="loader">Сохранённых AI вариантов пока нет</div>'}
...
if(a==='ai_history'){aiModalTitle.textContent='🖼 История AI этого поста';aiModal.classList.add('open');aiHistory.innerHTML='<div class="loader">Загрузка…</div>';try{const res=await fetch('/library/ai-history?id='+encodeURIComponent(id));const data=await res.json();if(!res.ok||!data.ok)throw new Error(data.error||'Ошибка');renderAiItems(data.items,id,'history')}catch(err){aiHistory.innerHTML='<div class="loader">'+String(err.message||'Ошибка')+'</div>'}return}
if(a==='ai_album'){aiModalTitle.textContent='🗂 Альбом AI · все сгенерированные фото';aiModal.classList.add('open');aiHistory.innerHTML='<div class="loader">Загружаю общий альбом…</div>';try{const res=await fetch('/library/ai-album');const data=await res.json();if(!res.ok||!data.ok)throw new Error(data.error||'Ошибка');renderAiItems(data.items,id,'album')}catch(err){aiHistory.innerHTML='<div class="loader">'+String(err.message||'Ошибка')+'</div>'}return}
...
async function libraryAiAlbum(env) {
  await ensurePublishingTables(env);
  const result = await env.mosaic_marketing_bot_db.prepare(`
    SELECT h.id, h.post_id, h.image_title, h.ai_check_status, h.ai_check_score,
           h.ai_check_summary, h.ai_check_issues_json, h.created_at, p.topic, p.ru_text
    FROM content_ai_history h
    LEFT JOIN content_posts p ON p.id = h.post_id
    ORDER BY h.id DESC
  `).all();
  const items = Array.isArray(result?.results) ? result.results.map(row => {
    const ru = splitRuPayload(row.ru_text);
    return {
      ...
      title:String(row.image_title||""),
      topic:String(ru.topic || row.topic || ""),
      createdAt:libraryDate(row.created_at),
      checkStatus:normalizeAiCheckStatus(row.ai_check_status),
      checkScore:row.ai_check_score == null ? null : normalizeAiCheckScore(row.ai_check_score),
      checkSummary:String(row.ai_check_summary||""),
...
async function handleLibraryAction(request, env) {
  ...
  try {
    ...
    if (action === "ai_delete_history") {
      const historyId = Number(payload.historyId || 0);
      if (!Number.isInteger(historyId) || historyId <= 0) return json({ok:false,error:"Некорректный AI вариант"},400);
      await ensurePublishingTables(env);
      const h = await env.mosaic_marketing_bot_db.prepare(`SELECT id,image_url FROM content_ai_history WHERE id=?`).bind(historyId).first();
      const ref = String(h?.image_url || "");
      if (!h || !ref.startsWith("tgfile:")) return json({ok:false,error:"AI фото уже удалено или не найдено"},404);
      const selected = await env.mosaic_marketing_bot_db.prepare(`SELECT post_id FROM content_media WHERE mode='ai' AND image_url=? ORDER BY post_id LIMIT 20`).bind(ref).all();
      const usedBy = Array.isArray(selected?.results) ? selected.results.map(x=>Number(x.post_id)).filter(x=>Number.isInteger(x)&&x>0) : [];
      if (usedBy.length) return json({ok:false,error:`Сначала выбери другое медиа или «Без фото» для пост${usedBy.length>1?'ов':'а'} #${usedBy.join(', #')}. Сейчас это AI фото используется как выбранное.`},409);
      await env.mosaic_marketing_bot_db.batch([
        env.mosaic_marketing_bot_db.prepare(`DELETE FROM content_ai_history WHERE id=?`).bind(historyId),
        env.mosaic_marketing_bot_db.prepare(`UPDATE content_media_previews SET ai_image_url=NULL,ai_image_title=NULL,ai_check_status=NULL,ai_check_score=NULL,ai_check_summary=NULL,ai_check_issues_json=NULL,updated_at=? WHERE ai_image_url=?`).bind(new Date().toISOString(),ref)
      ]);
      return json({ok:true,message:"AI фото удалено из альбома и D1"});
    }
    ...
    if (action === "ai_select_album") {
      const historyId = Number(payload.historyId||0);
      const h = await env.mosaic_marketing_bot_db.prepare(`SELECT image_url,image_title,ai_check_status,ai_check_score,ai_check_summary,ai_check_issues_json FROM content_ai_history WHERE id=?`).bind(historyId).first();
      if (!String(h?.image_url||"").startsWith("tgfile:")) return json({ok:false,error:"AI фото из альбома не найдено"},404);
      const checkStatus = normalizeAiCheckStatus(h.ai_check_status);
      if (checkStatus === "reject") return json({ok:false,error:"Это AI фото имеет статус Reject и заблокировано от выбора"},409);
      await saveMediaPreviewState(env,postId,{
        aiImageUrl:String(h.image_url),
        aiImageTitle:String(h.image_title||"AI album"),
        aiCheckStatus:checkStatus,
        aiCheckScore:h.ai_check_score,
        aiCheckSummary:String(h.ai_check_summary||""),
        aiCheckIssuesJson:String(h.ai_check_issues_json||"[]")
      ...
      const selected = await selectAiImage(env,postId,null);
      return json({ok:Boolean(selected?.ok),message:selected?.ok ? (selected.warning ? "Фото из общего AI-альбома выбрано с предупреждением" : "Фото из общего AI-альбома выбрано") : undefined,error:selected?.ok ? undefined : "Не удалось выбрать фото из AI-альбома"}, selected?.ok ? 200 : 409);
    }
