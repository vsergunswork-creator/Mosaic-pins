...
(function(){
    ...
    /* ✅ FIX race: keep instances + global render sequence */
...
/* ==========================================================
  ...
  function sanitizeCart(raw){
    ...
    for (const it of raw){
      ...
      const cleanQty = Number.isFinite(qty) ? Math.max(1, Math.floor(qty)) : 1;
      ...
      out.push({
        ...
        price: (it.price && typeof it.price === "object") ? it.price : {},
        stock: (it.stock == null) ? undefined : it.stock
      });
    }
    return out;
  }
  ...
  function writeCart(arr){
    try{ localStorage.setItem(CART_KEY, JSON.stringify(sanitizeCart(arr))); }catch(_){}
  }
  ...
  function refreshPriceUI(){
    if (currentProduct){
      const p = el("price");
      if (p) p.textContent = priceText(currentProduct.price);
    }
  }
  ...
  function resetPayPalHard(){
    ...
    try{ clearPayPalCartButtons(); }catch(_){}
    try{ clearPayPalOneButtons(); }catch(_){}
    try{ showPayPalCartNote(""); }catch(_){}
  ...
  function openCart(){
    ...
    setTimeout(() => { maybeInitPayPalCart(); }, 180);
  }
  ...
  function priceTextFromItem(it, cur){
    ...
    return cur === "EUR" ? `${unit.toFixed(2)} €` : `${unit.toFixed(2)} $`;
  }
...
if (elPpNote){
  if (!cart.length){
    elPpNote.style.display = "none";
    elPpNote.textContent = "";
  } else {
    // ✅ показываем только если PayPal реально грузится или ещё не готов
    const shouldShow = ppLoading || !ppReady;
    elPpNote.style.display = shouldShow ? "block" : "none";
    elPpNote.textContent   = shouldShow ? "PayPal is loading…" : "";
  }
}
    ...
    [...elCartBody.querySelectorAll(".cartItem")].forEach(row => {
      ...
      row.querySelector('button[data-act="minus"]')
        ?.addEventListener("click", () => setCartQty(pin, Number(input?.value) - 1));

      row.querySelector('button[data-act="plus"]')
        ?.addEventListener("click",  () => setCartQty(pin, Number(input?.value) + 1));
      ...
      row.querySelector('button[data-act="remove"]')
        ?.addEventListener("click", () => removeFromCart(pin));
    });
  ...
  elCartClear?.addEventListener("click", () => {
    clearCart();
    toast("Cart", "Cleared");
  });
  ...
  async function maybeInitPayPalCart(){
    try{
      ...
      ppLoading = true;
      showPayPalCartNote("PayPal is loading…");
      ...
      ppLoading = false;
      ppReady = false;
      showPayPalCartNote(`PayPal unavailable: ${String(e?.message || e)}`);
    }
  }
  ...
  function renderPayPalCartIfNeeded(){
    ...
    if (key === ppCartRenderedForKey) return;

    ppCartRenderedForKey = key;
    ...
    Promise.resolve().then(() => {
      ...
      ppCartButtons = window.paypal.Buttons({
      ...
      try{
        ppCartButtons.render("#paypal-cart-container");
      }catch(_){}
    });
  }
...
// ---- product block 4 ----
/* ==========================================================
     PayPal ONE item (product card) ✅ close() + seq
  ========================================================== */
  ...
  async function maybeInitPayPalOne(){
    ...
    const wrap = el("ppOneWrap");
    const note = el("ppOneNote");
    if (!wrap) return;
    ...
    if (!ppReady && !ppLoading){
      try{
        ppLoading = true;
        if (note){
          note.textContent = "PayPal is loading…";
        ...
        ppReady = false;
        if (note){
          note.textContent = `PayPal unavailable: ${String(e?.message || e)}`;
          note.style.display = "block";
        }
        return;
      }
    ...
    Promise.resolve().then(() => {
      ...
      const wrapEl = document.getElementById("paypal-one-container");
      if (!wrapEl) return;
      wrapEl.innerHTML = "";
      ...
      ppOneButtons = window.paypal.Buttons({
      ...
      try{
        ppOneButtons.render("#paypal-one-container");
      }catch(_){}
    });
  }
...
// ---- product block 5 ----
/* ==========================================================
     Carousel + Product load + Add-to-cart + Buy-now
  ========================================================== */
  ...
  function addToCart(p, qty){
    ...
    if (elCartDrawer?.classList.contains("show")){
      renderCart();
      ppCartRenderedForKey = "";
      setTimeout(()=> maybeInitPayPalCart(), 0);
    }
  ...
  async function loadProduct(){
    ...
    const oneWrap = el("ppOneWrap");
    const oneNote = el("ppOneNote");
    if (oneWrap) oneWrap.style.display = (stock > 0) ? "flex" : "none";
    if (oneNote){
      oneNote.textContent = (stock > 0) ? "PayPal is loading…" : "";
      oneNote.style.display = (stock > 0) ? "block" : "none";
    }
    ...
    el("minus").onclick = () => setQty(Number(qtyInput.value) - 1);
    el("plus").onclick  = () => setQty(Number(qtyInput.value) + 1);
    qtyInput.oninput    = () => setQty(qtyInput.value);

    el("price").textContent = priceText(p.price);
    refreshPriceUI();
